...
class Lateral_MPC_controller(object):
    ...
    def cal_error_k_fun(self, ts=0.01):
        ...
        for i in range(self.min_index, min(self.min_index + 50, path_length)):  # 当控制器是根据全局路径进行控制的时候，
            ...
            d = (self._target_path[i][0] - x) ** 2 + \
                (self._target_path[i][1] - y) ** 2
            if d < min_d:
                min_d = d
                self.min_index = i
        ...
        # 2.计算车身坐标系下的轴向向量和法向量
        tor_v = np.array([math.cos(self._target_path[min_index][2]),
                         math.sin(self._target_path[min_index][2])])
        n_v = np.array([-math.sin(self._target_path[min_index][2]),
                       math.cos(self._target_path[min_index][2])])

        # 3.计算匹配点指向实际位置的向量
        d_v = np.array([x - self._target_path[min_index][0],
                       y - self._target_path[min_index][1]])
        ...
        theta_r = self._target_path[min_index][2] + \
            self._target_path[min_index][3] * \
            e_s  # 认为投影点和匹配点航向不同，相差一个d_theta = k_r*d_s
        ...
        # 7.计算e_fi
        # e_fi = fi - theta_r
        # 这里为了防止角度的多值性，用sin(e_fi)近似e_fi， 一般情况下e_fi就是一个小量，所以可以近似
    ...
    def cal_control_para_fun(self, Q, R, F):
        ...
        for i in range(1, self._N + 1):
            M[i*self._n:(i+1)*self._n, :] = self.A_bar @ M[(i-1)
                                                           * self._n:i*self._n, :]

        C = np.zeros(shape=((self._N + 1) * self._n, self._N * self._P))
        # 这里self.B_bar是4*1维度的， 由于控制区间是self_P，因此这里有个按列复制的过程
        C[self._n:2*self._n, 0:self._P] = self.B_bar
        for i in range(2, self._N + 1):
            C[i * self._n:(i + 1) * self._n, (i-1) *
              self._P:i * self._P] = self.B_bar
            for j in range(i-2, -1, -1):
                C[i*self._n:(i+1)*self._n, j*self._P:(j+1)*self._P] = \
                    self.A_bar @ C[i*self._n:(i+1)*self._n,
                                   (j+1)*self._P:(j+2)*self._P]

        Cc = np.zeros(shape=((self._N+1)*self._n, 1))
        for i in range(1, self._N+1):
            Cc[self._n*i:self._n*(i+1), 0:1] = self.A_bar @ Cc[self._n *
                                                               (i-1):self._n*i, 0:1] + self.C_bar
        ...
        for i in range(self._N):
            R_bar[i*self._P:(i+1)*self._P, i*self._P:(i+1)
                  * self._P] = np.eye(self._P)*R
        ...
        H = C.T @ Q_bar @ C + R_bar
        E = C.T @ Q_bar @ Cc + \
            C.T @ Q_bar @ M @ (np.array(self.e_rr).reshape(self._n, 1))
        ...
        G = np.concatenate((np.identity(self._N*self._P), -
                           np.identity(self._N*self._P)))  # （4n, 2n）
...
class Lateral_LQR_controller(object):
    ...
    def xy_list_2_target_path(self, pathway_xy_list):
        ...
        for i in range(len(theta_list)):
            self._target_path.append(
                (pathway_xy_list[i][0], pathway_xy_list[i][1], theta_list[i], kappa_list[i]))
    ...
    def LQR_fun(self, Q, R):
        ...
        for i in range(max_itr):
            P = AT @ P @ A - (AT @ P @ B) @ np.linalg.inv(R +
                                                          BT @ P @ B) @ (BT @ P @ A) + Q  # 要不断迭代
    ...
    def cal_error_k_fun(self, ts=0.01):
        ...
        # delta_theta 就是切线转角，delta_s是弧长，
        # 我们要假设匹配点和投影点的航向并不相同，但是近似认为两点的曲率是相同的，这样才可以计算delta_theta
        # 认为投影点和匹配点航向不同，相差一个d_theta = k_r*d_s
        theta_r = self._target_path[min_index][2] + \
            self._target_path[min_index][3] * e_s
        # theta_r = self._target_path[min_index][2]  # delta_theta本质上就是一个很小的值，
        # apollo方案，将其近似为零，认为投影点和匹配点的航向角相同，这样是有误差的，个人更偏向于用不为零的近似
        ...
        # 8.计算S的导数
        S_dao = (V_x * math.cos(fi - theta_r) - V_y * math.sin(fi -
                 theta_r)) / (1 - self._target_path[min_index][3] * e_d)
    ...
    def forward_control_fun(self):
        ...
        K_3 = self.K[0][2]
        V_x = self._vehicle_Vx
        self.delta_f = self.k_r * \
            (a + b - b * K_3 - (b / Cf + a * K_3 / Cr - a / Cr)
             * (m * V_x * V_x) / (a + b))
...
class Longitudinal_PID_controller(object):
    ...
    def PID_fun(self):
        ...
        # 将新的误差放入缓存区，如果缓存区满了，最左边的溢出，整体数据左移一位，新的数据加在最右边
        ...
        if len(self.error_buffer) >= 2:
            ...
            # 微分误差，为了缓解超调
            differential_error = (
                self.error_buffer[-1] - self.error_buffer[-2]) / self.dt
        else:
            integral_error = 0.0
...
class Vehicle_control(object):
    def __init__(self, ego_vehicle, vehicle_para, pathway, controller_type="MPC_controller"):
        ...
        if controller_type == "MPC_controller":
            self.Lat_control = Lateral_MPC_controller(
                ego_vehicle, vehicle_para, pathway)
        elif controller_type == "LQR_controller":
            self.Lat_control = Lateral_LQR_controller(
                ego_vehicle, vehicle_para, pathway)

        self.Lon_control = Longitudinal_PID_controller(
            ego_vehicle)  # 这里不允许后续程序修改PID的参数，使用设定好的默认值
    ...
    def run_step(self, target_speed):
        ...
        if current_acceleration >= 0:
            ...
            control.throttle = 0
            # 没有反向加速，加速度为零时对应的是刹车制动
            control.brake = max(self._max_brake, current_acceleration)

        V = self._vehicle.get_velocity()
        V_len = math.sqrt(V.x * V.x + V.y * V.y + V.z * V.z)
        if print_flag:
            print("current speed:", V_len, "m/s",
                  "current steer control:", steering)
        return control
...
class Lateral_MPC__with_feedforward_controller(object):
    ...
    def cal_discretized_matrix(self):
        ...
        self.A_bar = temp @ (np.eye(4) + (ts * self.A) / 2)
        self.B_bar = temp @ self.B * ts
        # 这里将theta_r_dao看做常数，无飘移假设下theta_r_dao = k_r * Vx
        self.C_bar = temp @ self.C * ts * self.k_r * self._vehicle_Vx
    ...
    def cal_control_para_fun(self, Q, R, F):
        ...
        # 这里self.B_bar是4*1维度的， 由于控制区间是self_P，因此这里有个按复制的过程
        C[self._n:2*self._n, 0:self._P] = self.B_bar
        for i in range(2, self._N + 1):
            C[i * self._n:(i + 1) * self._n, (i-1) *
              self._P:i * self._P] = self.B_bar
        ...
        # 解决二次规划需要转化为标准形式0.5*x'.H.x + f'x
        H = 2 * H
        f = 2 * E.T @ (np.array(self.e_rr).reshape(self._n, 1)
                       ) + 2 * C.T @ Q_bar.T @ Cc
        ...
        # 计算时要将输入转化为cvxopt.matrix
        # 该方法返回值是一个字典类型，包含了很多的参数，其中x关键字对应的是优化后的解
        res = cvxopt.solvers.qp(cvxopt.matrix(H), cvxopt.matrix(
            f), G=cvxopt.matrix(G), h=cvxopt.matrix(h))
        return res['x'][0]
